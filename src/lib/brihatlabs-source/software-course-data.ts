...
function quizQuestions(moduleTitle: string, subject: SoftwareSubject): QuizQuestion[] {
  ...
  const contextual: [string, string[], number, string] = [`What is the strongest way to learn ${moduleTitle}?`, ["Build a small example, inspect evidence, and explain trade-offs", "Memorize names only", "Skip failure cases", "Avoid feedback"], 0, "A small observable example plus deliberate practice builds transferable understanding."];
  return [contextual, ...bank].map(([prompt, options, answer, explanation], index) => ({ id: `q${index + 1}`, prompt, options, answer, explanation }));
}
